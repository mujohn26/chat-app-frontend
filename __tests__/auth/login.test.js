...
import {
	LoginPage,
	mapStateToProps,
} from '../../src/views/auth/login.js';
import { props } from '../../__mockData__/loginMock';
...
const setUpComponent = (initialState = {}) => {
	const store = testStore(initialState);
	const wrapper = mount(<LoginPage {...props} store={store} />);
	wrapper.setState({
		password: 'mujohn25',
		userNme: 'mujohn25',
...
describe('Render Login components', () => {
    it('should be handle submit function', () => {
		...
		wrapper.setState({
        email:"mujohn25@gmail.com", password:"test@2021"
            	});
    ...
    it('should be handle signup function', () => {
		const wrapper = mount(<LoginPage {...props} />);
		wrapper.setState({
                    userName:"John", email:"mujohn25@gmail.com", password:"test@2021"
            	});
		const handleSubmitSpy = jest.spyOn(wrapper.instance(), 'handleGoToSignup');

		wrapper.instance().handleGoToSignup();
	});
